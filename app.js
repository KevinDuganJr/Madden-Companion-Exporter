...
app.post('/:username/:platform/:leagueId/leagueteams', (req, res) => {
  ...
  const dataRef = ref.child(`${leagueId}/leagueteams`);
...
app.post('/:username/:platform/:leagueId/standings', (req, res) => {
  ...
  const { params: { username } } = req;  
  const {platform, leagueId} = req.params;
  const dataRef = ref.child(`${leagueId}/standings`);
...
app.post('/:username/:platform/:leagueId/week/:weekType/:weekNumber/:dataType', (req, res) => {
  ...
  const dataRef = ref.child(`${leagueId}/week/${weekType}/${weekNumber}/${dataType}`);
...
app.post('/:username/:platform/:leagueId/freeagents/roster', (req, res) => {
  ...
  const { params: { username } } = req;  
  const {platform, leagueId} = req.params;
  const dataRef = ref.child(`${leagueId}/freeagents`);
...
app.post('/:username/:platform/:leagueId/team/:teamId/roster', (req, res) => {
  ...
  const dataRef = ref.child(`${leagueId}/team/${teamId}`);
  const {body: {rosterInfoList}} = req;
  res.sendStatus(202);
  dataRef.set({
    rosterInfoList
